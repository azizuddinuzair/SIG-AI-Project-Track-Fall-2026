...
def compute_derived_features(df):
    ...
    # 1️⃣ RATIOS & COMPOSITE STATS (Raw scales, will normalize later)
    ...
    # Physical bulk: HP × DEF (raw, not percentage)
    df['Bulk_Physical'] = df['hp'] * df['defense']
    
    # Special bulk: HP × SPA (raw, not percentage)
    ...
    # 3️⃣ INTERACTION FEATURES (Encode Role Semantics)
...
def save_enhanced_features(df_enhanced):
    ...
    print(f"✅ Saved enhanced features to: pokemon_archetype_features_enhanced.csv")
...
def print_feature_summary(df_enhanced):
    ...
    print("1️⃣  RATIOS & COMPOSITE STATS")
...
def main():
    ...
    print(f"✅ Loaded {df.shape[0]} Pokémon with {df.shape[1]} initial features\n")
    
    # Compute derived features
    df_with_derived = compute_derived_features(df)
    print(f"✅ Computed 11 derived features\n")
    
    # Normalize derived features
    df_enhanced = normalize_features(df_with_derived)
    print(f"✅ Normalized derived features\n")
